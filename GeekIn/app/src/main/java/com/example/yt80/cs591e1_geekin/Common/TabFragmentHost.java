...
public class TabFragmentHost extends TabHost implements
    ...
    public void setup(Context context, FragmentManager manager, int containerId, User user) {
        ensureHierarchy(context); // Ensure views required by super.setup()
        super.setup();
        this.user = user;
        mContext = context;
    ...
    private void ensureContent() {//must be a <? extends FrameLayout> yt80
    ...
    public void onTabChanged(String tabId) {
        if (mAttached) {
            if (tabId == "3") {
                Log.i("new post", "ontab changede aaaaaaaaaaaaaaaa");
                Fragment new_post = Fragment.instantiate(mContext,
                        NewPostFragMain.class.getName(), new Bundle());
                FragmentTransaction ft = mFragmentManager.beginTransaction();
                ft.setCustomAnimations(R.anim.slide_up, R.anim.fade_out,R.anim.fade_in, R.anim.slide_down);
                ft.replace(mContainerId,new_post,"new_post");
                ft.addToBackStack(null);
                ft.commit();
                //setCurrentTab(Integer.parseInt(mLastTab.tag)-1);
    ...
    public View getBorderLine() {
        View borderLine = findViewById(R.id.borderLine);
        return borderLine;
    }
